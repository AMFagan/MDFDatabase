...
class Course(models.Model):
    ...
    def levels(self):
        ...
        for ls in out:
            ...
            if i == 0:
                del out[ls][sem]
        """
        return {k: v for k, v in sorted(out.items(), key=lambda item: item[0])}
    ...
    def all_classes(self):
        ...
        for mem in self.coursemembership_set.all():
            if mem.level not in out:
                out[mem.level] = []
            out[mem.level] += [mem.module]
        return {k: v for k, v in sorted(out.items(), key=lambda item: item[0])}
            
    def get_level(self, year):
        out = []
        for mem in self.coursemembership_set.all():
            if mem.level == year:
                out += [mem]
        out.sort(key=lambda mem: (mem.elective, mem.module.name))
        return out
